...
from RL2.utils.comm import gather_and_concat_list, split_and_scatter_list
from RL2.utils.logging import gather_and_log, time_logger
    ...
    def update(self, actor, step):
        ...
        for idx, (name, tensor) in enumerate(state_dict.items()):
            # if name == 'model.layers.0.self_attn.q_proj.weight':
                # print(f' RANK {dist.get_rank()} tensor: {tensor} tensor shape {tensor.to_local().shape} \n\n')
            
            tensor = tensor.to(torch.cuda.current_device())
            # new_tensor = tensor.full_tensor() if isinstance(tensor, DTensor) else tensor
            ...
            if name == 'model.layers.0.self_attn.q_proj.weight':
                print(f' RANK {dist.get_rank()} tensor shape serialized_tensors len {len(serialized_tensors)} device-mehs TP size {self.device_mesh["tp"].size()} \n\n')
